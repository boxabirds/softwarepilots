...
import type { LearnerProfile } from "@softwarepilots/shared";
...
const TEST_PROFILE: LearnerProfile = "business-leader";
const TEST_META = getCurriculumMeta(TEST_PROFILE);
const TEST_SECTION = getSection(TEST_PROFILE, TEST_META.sections[0].id);
...
describe("buildSocraticTools", () => {
  ...
  it("includes topic scope in tool descriptions", () => {
    const tools = buildSocraticTools(TEST_SECTION, TEST_META);
    const probeTool = tools[0].functionDeclarations.find(
      (d) => d.name === "socratic_probe"
    );
    for (const topic of TEST_SECTION.topics) {
      expect(probeTool?.description).toContain(topic);
    }
  });
});
...
describe("buildSocraticSystemPrompt", () => {
  it("includes tutor_guidance for each profile", () => {
    for (const profile of getAllProfiles()) {
      ...
      const section = getSection(profile, meta.sections[0].id);
