...
describe("PUT conversation upsert with real SQLite", () => {
  it("inserts a new conversation", () => {
    ...
    db.prepare(sql).run("learner-1", "level-1", "1.1", '[{"role":"user","content":"hello"}]');
    ...
    ).get("learner-1", "level-1", "1.1") as { messages_json: string } | undefined;
  ...
  it("upserts an existing conversation", () => {
    ...
    db.prepare(sql).run("learner-1", "level-1", "1.1", '[{"role":"user","content":"first"}]');
    db.prepare(sql).run("learner-1", "level-1", "1.1", '[{"role":"user","content":"second"}]');

    const rows = db.prepare(
      "SELECT messages_json FROM curriculum_conversations WHERE learner_id = ? AND profile = ? AND section_id = ?"
    ).all("learner-1", "level-1", "1.1") as { messages_json: string }[];
  ...
  it("does not conflict with archived conversations", () => {
    ...
    ).run("old-id", "learner-1", "level-1", "1.1", '[{"role":"user","content":"old"}]');
    ...
    db.prepare(sql).run("learner-1", "level-1", "1.1", '[{"role":"user","content":"new"}]');
    ...
    ).all("learner-1", "level-1", "1.1") as { messages_json: string; archived_at: string | null }[];
  ...
  it("works through the curriculum route handler via D1 shim", async () => {
    ...
    const res1 = await app.request("/level-1/1.1/conversation", {
      method: "PUT",
    ...
    const res2 = await app.request("/level-1/1.1/conversation", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        messages: [
    ...
    const row = sqliteDb.prepare(
      "SELECT messages_json FROM curriculum_conversations WHERE learner_id = ? AND profile = ? AND section_id = ? AND archived_at IS NULL"
    ).get("learner-1", "level-1", "1.1") as { messages_json: string };
  ...
  it("creates new active conversation after archive", async () => {
    ...
    await app.request("/level-1/1.1/conversation", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
    ...
    const res = await app.request("/level-1/1.1/conversation", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
